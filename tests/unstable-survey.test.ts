...
test(title, details, async ({ page }) => {
  ...
  // Retrying to enter a randomized email address in the correct field.
  ...
  // Re-entering the full name to proceed with the survey as it wasn't successful earlier.
  await page.inputText({
    text: 'John Doe',
    finalizeWithSubmit: false,
    selector: {
  ...
  // Selecting an age group to proceed with the survey.
  ...
  // Selecting the age group '18-24' to proceed with the survey.
  await page.clickElement({
    selector: {
      element: [
  ...
  // Selecting 'Social Media' as the source of hearing about the survey.
  await page.clickElement({
    selector: {
      element: [
        '#\\:r13\\:-form-item',
  ...
  // Clicking the 'Next' button to proceed with the survey.
  await page.clickElement({
    selector: {
      element: [
  ...
  // Verifying that the survey submission was successful with a thank you message displayed.
